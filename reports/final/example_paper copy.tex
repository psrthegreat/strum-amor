...
\twocolumn[
\icmltitle{A Supervised Approach To Chord Recognition}
...
\begin{abstract} 
In this paper, we present Strumlet, a prototype of an online chord recognition system.
\end{abstract} 

\section{Introduction}
\label{intro}

Why is chord recognition important.
a- to listener in identifying a song
b. learner to play a song, get chords from pop songs. Auto transcription. Chords are easy to play along to.
c. to a learner in evaluating how well he/she is playing a chord.
d. for writers to find good chords to accompany melodies.

\subsection{previous work}

previous work melody finding. genre classification
chord detection harder. people have done hidden markov models,
-
-
they have used knowledge specific to a certain genre
...
generating data
- major and minor 24 chords. triads.

feature extraction
- mfcc to chroma to crp

extracting features from wav files
- first same length
- concatenated all of the frames

then wanted to handle different sized wave files.
looked at a wav files as a list of frames of a certain window size.
a wav file then became a f * n matrix.

putting frames together
- first imposed constraint that chords could not change in a wav file.
So we used naive bayes to predict which chord it was.

results numbers

-then we used hidden markov models. viterbi algorithm to figure out most likely chord sequence
emission probabilities.
- 

testing pipeline
results on clean chords
- 99 %...

-live so needed to be optimized for speed.
100ms
500 ms wav file gave best compromise between speed and accuracy.
